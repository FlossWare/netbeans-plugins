...
/**
 * Service layer for Cohere API interactions
 * Handles async operations and thread management
 */
public class CohereService {

    private static CohereService instance;
    private final CohereClient client;
    private final RequestProcessor requestProcessor;

    private CohereService() {
        this.client = new CohereClient();
        this.requestProcessor = new RequestProcessor("Cohere API", 3);
    }

    public static synchronized CohereService getInstance() {
        if (instance == null) {
            instance = new CohereService();
        }
        return instance;
    }
