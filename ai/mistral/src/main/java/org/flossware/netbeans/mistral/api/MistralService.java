...
/**
 * Service layer for Mistral API interactions
 * Handles async operations and thread management
 */
public class MistralService {

    private static MistralService instance;
    private final MistralClient client;
    private final RequestProcessor requestProcessor;

    private MistralService() {
        this.client = new MistralClient();
        this.requestProcessor = new RequestProcessor("Mistral API", 3);
    }

    public static synchronized MistralService getInstance() {
        if (instance == null) {
            instance = new MistralService();
        }
        return instance;
    }
     ...
     */
    public MistralClient getClient() {
        return client;
    }
