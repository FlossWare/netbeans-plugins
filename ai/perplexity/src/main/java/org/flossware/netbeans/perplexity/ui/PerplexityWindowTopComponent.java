...
/**
 * Top component which displays Perplexity chat window
 */
@TopComponent.Description(
        preferredID = "PerplexityWindowTopComponent",
        iconBase = "org/flossware/netbeans/perplexity/resources/perplexity-icon.png",
        persistenceType = TopComponent.PERSISTENCE_ALWAYS
)
@TopComponent.Registration(mode = "rightSlidingSide", openAtStartup = false)
@ActionID(category = "Window", id = "org.flossware.netbeans.perplexity.ui.PerplexityWindowTopComponent")
@ActionReference(path = "Menu/Window", position = 335)
@TopComponent.OpenActionRegistration(
        displayName = "#CTL_PerplexityWindowAction",
        preferredID = "PerplexityWindowTopComponent"
)
@Messages({
    "CTL_PerplexityWindowAction=Perplexity AI",
    "CTL_PerplexityWindowTopComponent=Perplexity AI Chat",
    "HINT_PerplexityWindowTopComponent=Perplexity AI Assistant"
})
public final class PerplexityWindowTopComponent extends TopComponent {

    private static PerplexityWindowTopComponent instance;
    ...
    public PerplexityWindowTopComponent() {
        this.perplexityService = PerplexityService.getInstance();
        initComponents();
        setName(Bundle.CTL_PerplexityWindowTopComponent());
        setToolTipText(Bundle.HINT_PerplexityWindowTopComponent());
    }
    ...
    private void initComponents() {
        ...
        appendToChat("System", "Welcome to Perplexity AI Assistant!\n" +
    ...
    private void sendMessage(ActionEvent evt) {
        ...
        if (!perplexityService.isConfigured()) {
            appendToChat("System",
                    "API key not configured. Please set your Perplexity API key in Tools > Options.",
                    Color.RED);
        ...
        appendToChat("Perplexity", "Thinking...", Color.GRAY);
        perplexityService.sendMessageAsync(message).thenAccept(response -> {
            SwingUtilities.invokeLater(() -> {
                removeLastMessage(); // Remove "Thinking..."
                appendToChat("Perplexity", response, new Color(32, 164, 243));
                sendButton.setEnabled(true);
    ...
    private void removeLastMessage() {
        ...
        int lastIndex = text.lastIndexOf("Perplexity: Thinking...");
        if (lastIndex != -1) {
            try {
    ...
    public static synchronized PerplexityWindowTopComponent getDefault() {
        if (instance == null) {
            instance = new PerplexityWindowTopComponent();
        }
        return instance;
    }

    public static synchronized PerplexityWindowTopComponent findInstance() {
        TopComponent win = WindowManager.getDefault().findTopComponent("PerplexityWindowTopComponent");
        if (win == null) {
            return getDefault();
        }
        if (win instanceof PerplexityWindowTopComponent) {
            return (PerplexityWindowTopComponent) win;
        }
        return getDefault();
    }
    ...
    protected String preferredID() {
        return "PerplexityWindowTopComponent";
    }
    ...
    /**
     * Send a message to Perplexity asynchronously
     */
    private void sendToPerplexityAsync(String message) {
        sendButton.setEnabled(false);
