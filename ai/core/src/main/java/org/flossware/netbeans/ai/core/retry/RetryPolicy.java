...
package org.flossware.netbeans.ai.core.retry;
...
import org.flossware.netbeans.ai.core.exceptions.RateLimitException;

/**
 * Retry policy with exponential backoff and jitter for handling transient failures.
 *
 * <p>Implements best practices for API retry logic including:</p>
 * <ul>
 *   <li>Exponential backoff to prevent overwhelming the server</li>
 *   <li>Jitter to prevent thundering herd problem</li>
 *   <li>Configurable retry attempts and backoff</li>
 *   <li>Special handling for rate limiting (429) responses</li>
 * </ul>
 */
public class RetryPolicy {
     ...
     * @throws AIException If all retries are exhausted
     */
    public <T> T executeWithRetry(Callable<T> operation) throws AIException {
        ...
        for (int attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (e instanceof AuthException || e instanceof ConfigException) {
