...
import org.flossware.netbeans.grok.api.GrokService;
...
/**
 * Top component which displays Grok chat window
 */
@TopComponent.Description(
        preferredID = "GrokWindowTopComponent",
        iconBase = "org/flossware/netbeans/grok/resources/grok-icon.png",
        persistenceType = TopComponent.PERSISTENCE_ALWAYS
)
@TopComponent.Registration(mode = "rightSlidingSide", openAtStartup = false)
@ActionID(category = "Window", id = "org.flossware.netbeans.grok.ui.GrokWindowTopComponent")
@ActionReference(path = "Menu/Window", position = 334)
@TopComponent.OpenActionRegistration(
        displayName = "#CTL_GrokWindowAction",
        preferredID = "GrokWindowTopComponent"
)
@Messages({
    "CTL_GrokWindowAction=Grok",
    "CTL_GrokWindowTopComponent=Grok Chat",
    "HINT_GrokWindowTopComponent=Grok AI Assistant"
})
public final class GrokWindowTopComponent extends TopComponent {

    private static GrokWindowTopComponent instance;
    ...
    public GrokWindowTopComponent() {
        this.grokService = GrokService.getInstance();
        initComponents();
        setName(Bundle.CTL_GrokWindowTopComponent());
        setToolTipText(Bundle.HINT_GrokWindowTopComponent());
    }
    ...
    private void initComponents() {
        ...
        appendToChat("System", "Welcome to Grok AI Assistant!\n" +
    ...
    private void sendMessage(ActionEvent evt) {
        ...
        if (!grokService.isConfigured()) {
            appendToChat("System",
                    "API key not configured. Please set your xAI API key in Tools > Options.",
                    Color.RED);
        ...
        // Send to Grok API
        appendToChat("Grok", "Thinking...", Color.GRAY);
        grokService.sendMessageAsync(message).thenAccept(response -> {
            SwingUtilities.invokeLater(() -> {
                removeLastMessage(); // Remove "Thinking..."
                appendToChat("Grok", response, new Color(0, 102, 204));
                sendButton.setEnabled(true);
    ...
    private void removeLastMessage() {
        ...
        int lastIndex = text.lastIndexOf("Grok: Thinking...");
        if (lastIndex != -1) {
            try {
    ...
    public static synchronized GrokWindowTopComponent getDefault() {
        if (instance == null) {
            instance = new GrokWindowTopComponent();
        }
        return instance;
    }

    public static synchronized GrokWindowTopComponent findInstance() {
        TopComponent win = WindowManager.getDefault().findTopComponent("GrokWindowTopComponent");
        if (win == null) {
            return getDefault();
        }
        if (win instanceof GrokWindowTopComponent) {
            return (GrokWindowTopComponent) win;
        }
        return getDefault();
    }
    ...
    protected String preferredID() {
        return "GrokWindowTopComponent";
    }

    /**
     * Ask Grok about selected code
     */
    public void askAboutCode(String code) {
        String message = "Can you explain this code?\n\n```\n" + code + "\n```";
        appendToChat("You", message, Color.BLACK);
        sendToGrokAsync(message);
    }
    ...
    /**
     * Send a message to Grok asynchronously
     */
    private void sendToGrokAsync(String message) {
        sendButton.setEnabled(false);
