...
public class CompletionCache {
     ...
     */
    public List<ChatGPTCompletionItem> get(String key) {
     ...
     */
    public void put(String key, List<ChatGPTCompletionItem> items) {
        int ttlSeconds = ChatGPTCompletionSettings.getCacheTTLSeconds();
    ...
    private void startCleanupTask() {
        ...
        cleanupThread.setName("ChatGPT-Completion-Cache-Cleanup");
        cleanupThread.start();
    }
    ...
    private static class CacheEntry {
        private final List<ChatGPTCompletionItem> items;
        private final long expirationTime;

        public CacheEntry(List<ChatGPTCompletionItem> items, int ttlSeconds) {
            this.items = items;
            this.expirationTime = System.currentTimeMillis() + (ttlSeconds * 1000L);
        }

        public List<ChatGPTCompletionItem> getItems() {
            return items;
        }
