...
/**
 * Manages BeanShell debugging sessions using JDWP
 */
public class BeanShellDebuggerSession {

    private final File scriptFile;
    private Process debugProcess;
    private static final int DEBUG_PORT = 5006;

    public BeanShellDebuggerSession(File scriptFile) {
    ...
    public void start() {
        try {
            String bshPath = BeanShellSettings.getInstance().getBeanShellPath();
            ...
            InputOutput io = IOProvider.getDefault().getIO("BeanShell Debugger: " + scriptFile.getName(), false);
            io.select();

            io.getOut().println("Starting BeanShell debugger...");
            ...
            InputOutput io = IOProvider.getDefault().getIO("BeanShell Debugger Error", false);
            io.select();
