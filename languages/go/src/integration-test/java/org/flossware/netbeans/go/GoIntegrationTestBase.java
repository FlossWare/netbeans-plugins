...
package org.flossware.netbeans.go;
...
/**
 * Base class for Go plugin integration tests.
 *
 ...
 * @since 1.0
 */
public abstract class GoIntegrationTestBase extends NbTestCase {

    protected File workDir;
    protected FileObject workDirFileObject;

    public GoIntegrationTestBase(String name) {
        super(name);
    }
    ...
    /**
     * Create a Go file in the work directory.
     *
     * @param name File name (e.g., "test.go")
     * @param content File content
     * @return FileObject representing the created file
     * @throws IOException if file creation fails
     */
    protected FileObject createGoFile(String name, String content) throws IOException {
     ...
     * @return FileObject for the created program
     * @throws IOException if file creation fails
     */
    protected FileObject createSimpleGoProgram(String fileName) throws IOException {
        String content = "package main\n\n"
                + "import \"fmt\"\n\n"
                + "func main() {\n"
                + "    fmt.Println(\"Hello from Go\")\n"
                + "}\n";
        return createGoFile(fileName, content);
    }
     ...
     */
    protected FileObject createGoProject(String projectName) throws IOException {
        FileObject projectDir = workDirFileObject.createFolder(projectName);

        // Create go.mod to make it a Go module project
        FileObject goMod = projectDir.createData("go.mod");
        String goModContent = "module " + projectName + "\n\n"
                + "go 1.21\n";
        try (OutputStream os = goMod.getOutputStream();
             PrintWriter pw = new PrintWriter(os)) {
            pw.print(goModContent);
        }

        // Create a Go source file
        FileObject mainGo = projectDir.createData("main.go");
        String mainContent = "package main\n\n"
                + "import \"fmt\"\n\n"
                + "func main() {\n"
                + "    fmt.Println(\"Hello from " + projectName + "\")\n"
                + "}\n";
        try (OutputStream os = mainGo.getOutputStream();
             PrintWriter pw = new PrintWriter(os)) {
