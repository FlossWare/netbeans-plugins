...
public class CompletionCache {
     ...
     */
    public List<ClaudeCompletionItem> get(String key) {
     ...
     */
    public void put(String key, List<ClaudeCompletionItem> items) {
        int ttlSeconds = ClaudeCompletionSettings.getCacheTTLSeconds();
    ...
    private void startCleanupTask() {
        ...
        cleanupThread.setName("Claude-Completion-Cache-Cleanup");
        cleanupThread.start();
    }
    ...
    private static class CacheEntry {
        private final List<ClaudeCompletionItem> items;
        private final long expirationTime;

        public CacheEntry(List<ClaudeCompletionItem> items, int ttlSeconds) {
            this.items = items;
            this.expirationTime = System.currentTimeMillis() + (ttlSeconds * 1000L);
        }

        public List<ClaudeCompletionItem> getItems() {
            return items;
        }
